...
def build_system_prompt(p: Preset) -> str:
    return (
        ...
        f"- Keep the answer under ~{p.max_words_hint} words unless absolutely necessary.\n"
        "- Be direct, actionable, and safety-conscious.\n"
        "- If key information is missing, explicitly list what is needed to determine stability.\n"
        "- Do NOT output hidden reasoning, chain-of-thought, or internal tags.\n"
        "- Never output '<unused94>thought'.\n"
        "- Output only the final answer.\n"
        "\n"
        "Output format (STRICT):\n"
        "Status: STABLE | WORSENING | UNCERTAIN\n"
        "Why: 1–3 short bullet points using only observable facts from the input\n"
        "Immediate Actions: 2–5 bullet points (prioritize ABCs, monitoring, escalation)\n"
        "Red Flags: 0–5 bullet points (only if present or high-risk based on input)\n"
        "Missing Data: bullet list (only if Status=UNCERTAIN or confidence is limited)\n"
    )
